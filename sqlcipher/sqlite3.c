...
/* Weak, so sqlite-wasm-rs can define these later, and until then cipher_log, cipher_profile and cipher_migrate fail and atexit is a no-op. */
#if defined(__wasm__)
__attribute__((weak)) FILE *const stdout = 0;
__attribute__((weak)) FILE *const stderr = 0;
__attribute__((weak)) FILE *fopen(const char *restrict path, const char *restrict mode) { (void)path; (void)mode; return 0; }
__attribute__((weak)) int fprintf(FILE *restrict f, const char *restrict fmt, ...) { (void)f; (void)fmt; return -1; }
__attribute__((weak)) int rename(const char *from, const char *to) { (void)from; (void)to; return -1; }
__attribute__((weak)) int atexit(void (*fn)(void)) { (void)fn; return 0; }
#endif
